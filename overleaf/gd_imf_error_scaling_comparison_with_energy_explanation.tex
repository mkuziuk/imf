...
The notebook \href{https://github.com/mkuziuk/imf/blob/main/experiments/real-vs-calculated/gd_imf_real_vs_calculated.ipynb}{\texttt{gd\_imf\_real\_vs\_calculated.ipynb}} scales every
stage-$k$ error by the geometric factor $a^{k/2}$, where $a=\sqrt{2}$ is the
bandwidth reduction factor. The companion notebook
\href{https://github.com/mkuziuk/imf/blob/main/experiments/real-vs-calculated/gd_imf_real_vs_calculated_energy_scaled.ipynb}{\texttt{gd\_imf\_real\_vs\_calculated\_energy\_scaled.ipynb}}, stored with the
other real-vs-calculated experiments, uses a stage-specific error scale derived from
...
In Nadaraya's note, Theorem~1 (starting on p.~141 and continuing at the top of
p.~142) gives the random-design $\sqrt{nh}$ limit and displays the corresponding
kernel-energy factor $\int K^2$.  The present experiment instead has a regular
fixed design.  For that design, a normalized kernel average with bandwidth $h$
...
\subsection{Why the first recursive operator has greater energy}

The difference is a consequence of the initialization of the recursion, rather
than of the bandwidth schedule alone. On the periodic regular grid used in the
notebooks, the smoothers are circulant. Let $w_k(\omega_m)$ be the discrete
Fourier multiplier of $W_k$. The multiplier and squared row energy of the
complete stage operator are
\[
q_k(\omega_m)
=w_k(\omega_m)\prod_{j<k}\{1-w_j(\omega_m)\},
\qquad
E_k=\lVert\ell_k\rVert_2^2
=\frac{1}{n}\sum_m |q_k(\omega_m)|^2.
\]
Thus operator energy is the integrated squared gain: it measures how much white
noise power the entire stage transmits. More precisely, the stage-one spike
after geometric scaling is controlled by the bandwidth-normalized energy
$C_k=(nh_k)E_k$. It is this constant, rather than necessarily the unnormalized
$E_k$ for every increasingly narrow later filter, that is exceptionally large
at stage one.

For $k=1$, the product is empty, so $q_1=w_1$. The first stage therefore keeps
the full low-frequency passband of a normalized low-pass smoother. In
particular, $w_1(0)=1$, and $|w_1(\omega)|$ remains close to one throughout a
neighborhood of zero frequency. It is this whole low-frequency neighborhood,
not the single DC Fourier coefficient by itself, that contributes the large
energy.

For every $k>1$, at least one residual factor is present. Since every
normalized smoother preserves constants, $w_j(0)=1$, and hence
\[
q_k(0)=w_k(0)\prod_{j<k}\{1-w_j(0)\}=0.
\]
More generally, wherever an earlier low-pass multiplier $w_j(\omega)$ is close
to one, the factor $1-w_j(\omega)$ is close to zero. The earlier residual
operators therefore remove the low-frequency region in which the current
smoother would otherwise have its largest gain. At the other end of the
spectrum, $w_k(\omega)$ is already small. The product can consequently transmit
mainly the narrower transition region between these two regimes: later
components are band-pass details, rather than low-pass averages of the original
data. The first residual factor creates most of the drop, and the older factors
provide further suppression.

The same distinction appears in the equivalent-kernel constant. If
$L=K/\int K$ and $h_j/h_k\approx a^{k-j}$, then, up to the Fourier convention,
\[
C_k=\frac{1}{2\pi}\int |\widehat L(z)|^2
\prod_{\ell=1}^{k-1}|1-\widehat L(a^\ell z)|^2\,dz.
\]
At stage one there are no attenuating factors. For the implemented kernel
$K(u)=\tfrac34(1-|u|)^2_+$,
\[
C_1=\int\left(\frac{K(u)}{\int K}\right)^2du=0.9.
\]
In the exact finite-$n$ calculation using the notebook's discrete windows,
$C_k=(nh_k)E_k$ equals $0.9009$ at stage 1, $0.1518$ at stage 2, $0.1090$ at
stage 3, and approximately $0.09$ thereafter. Equivalently, the residual
product leaves $16.8\%$ of the corresponding single-pass variance at stage 2
and about $9.7\%$--$12.1\%$ at later stages. This calculation is specific to
the implemented kernel, window schedule, and periodic boundary; the qualitative
low-pass-versus-band-pass mechanism is general, but a tenfold energy ratio is
not a universal IMF constant.

For the robust recursion there is no single fixed global Fourier multiplier.
Nevertheless, the local Jacobian of a converged normalized robust smoother has
rows summing to one, so its first-stage linearization preserves constants while
the Jacobian of each residual map removes them. This gives the same
low-pass-versus-band-pass explanation locally. The robust energy ratio in this
document is therefore supported by the bootstrap experiment, rather than by the
exact linear identity above.

\subsection{Sensitivity to the bandwidth schedule}

To check that the stage-one result is not an artifact of the single schedule
used above, the energy-scaled notebook varies the initial discrete radius
and the bandwidth shrink factor over
\[
\begin{aligned}
R_1&\in\{100,150,200,250,300,350\},\\
a&\in\{1.20,1.30,\sqrt{2},1.60,2.00,2.50,3.00,3.50,4.00,4.50,5.00\}.
\end{aligned}
\]
For the regular design with \(n=1000\), these radii correspond to normalized
half-widths \(h_1=R_1/(n-1)\), approximately \(0.10\) through \(0.35\).
Every schedule uses the existing odd-window rounding rule and stops at window
size \(31\), or radius \(15\). The current setting
\((R_1,a)=(250,\sqrt{2})\) is included in the 66-point grid.

For each linear schedule, the notebook computes both the raw operator energy
\(E_k=\lVert\ell_k\rVert_2^2\) and the bandwidth-normalized energy
\(C_k=(nh_k)E_k\). The strict comparison is
\[
\rho_C=\frac{C_1}{\max_{k\geq2}C_k}.
\]
Across the complete grid, \(\rho_C\) ranges from \(1.613\) to \(8.152\).
Its minimum occurs at \(R_1=100\) with the most aggressive shrink factor
\(a=5\), while the current schedule gives \(5.936\). Thus \(C_1\) exceeds every
later \(C_k\) at all 66 tested settings.

The corresponding raw-energy ratio
\(\rho_E=E_1/\max_{k\geq2}E_k\) ranges from \(0.072\) to \(3.725\) and equals
\(0.564\) at the current setting. Consequently, the robust conclusion is about
the bandwidth-normalized constant \(C_k\), not a universal ordering of the raw
energies \(E_k\). Narrow later filters can have larger raw energy even though
their bandwidth-normalized energy remains below the first-stage value.

\begin{figure}[H]
\centering
\includegraphics[width=\linewidth]{figures/error-scaling/energy_schedule_sensitivity_linear.png}
\caption{Exact linear sensitivity over the initial-radius and shrink-factor
grid. Each cell reports the first-stage energy divided by the largest later-stage
energy. The star marks the current schedule. The left panel uses the
bandwidth-normalized energy \(C_k\); the right panel shows the raw energy \(E_k\)
and demonstrates why the two claims must be distinguished.}
\end{figure}

The nonlinear robust calculation is screened at the current setting and the
four corners of the grid. Each setting uses the same 16 bootstrap seeds under
the notebook's Gaussian-plus-contamination model. The interval endpoints below
are percentile resampling intervals for this finite 16-replicate screen; they
describe Monte Carlo sensitivity and are not formal population confidence
intervals.
...
\begin{tabular}{@{}lrrr@{}}
\hline
Setting & \(R_1\) & \(a\) & \(C_1/\max_{k\geq2}C_k\) \\
\hline
Current & 250 & \(\sqrt{2}\) & \(5.08\ [3.42,7.42]\) \\
Small radius, gradual shrink & 100 & \(1.20\) & \(9.31\ [7.72,11.28]\) \\
Small radius, aggressive shrink & 100 & \(5.00\) & \(1.94\ [1.61,2.28]\) \\
Large radius, gradual shrink & 350 & \(1.20\) & \(6.88\ [4.13,10.63]\) \\
Large radius, aggressive shrink & 350 & \(5.00\) & \(1.43\ [0.83,2.22]\) \\
\hline
\end{tabular}
\caption{Robust bandwidth-normalized first-to-later energy ratios. Brackets
show exploratory 95\% Monte Carlo intervals from resampling the 16 matched
bootstrap replicates.}
\end{table}

\begin{figure}[H]
\centering
\includegraphics[width=\linewidth]{figures/error-scaling/energy_schedule_sensitivity_robust.png}
\caption{Robust first-stage bandwidth-normalized energy relative to the largest
later-stage value at the current schedule and the four grid corners. The dashed
line marks equality. All point estimates remain above one, but the exploratory
interval at \(R_1=350,a=5\) crosses equality.}
\end{figure}

The exact linear grid therefore shows that the first-stage ordering is stable
throughout the tested range. The smaller robust screen supports the same
ordering in its point estimates under nonlinear fitting and contamination.
However, uncertainty at the large-radius, \(a=5\) corner includes equality, so
a denser robust grid or more bootstrap replicates would be needed to resolve
that boundary case.
...
\begin{figure}[H]
\centering
\includegraphics[width=\linewidth]{figures/error-scaling/recursive_rmse_scaling_comparison.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.95\linewidth]{figures/error-scaling/recursive_flatness_comparison.png}
...
\begin{itemize}
    ...
    \href{https://github.com/mkuziuk/imf/blob/main/experiments/real-vs-calculated/gd_imf_real_vs_calculated_energy_scaled.ipynb}{\texttt{gd\_imf\_real\_vs\_calculated\_energy\_scaled.ipynb}}.
...
local copy: \path{nadaraya1964.pdf}.
...
\href{https://dtic.minsky.ai/index/AD0467119}{DTIC technical-report record
AD0467119}.
